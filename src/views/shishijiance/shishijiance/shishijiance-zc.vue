          ...
          if (data.transducer) {    
            ...
            for(let i=0;i<data.camera.length;i++) {
              if (data.camera[i].isOnline == '在线') {
                cameraList.push(data.camera[i]);
              }
            }
            let scStatus = cameraList[0].signalInfo == 0?'未生产视频':'生产视频';
            if (_this.isShowNow) {
              _this.getDataNow();
              _this.showVideo(scStatus);
            } else {
      ...
      return shipinApi.getByFactorDataNow(param)
        .then(data => {
          ...
          for (let i=0;i<data.data.length;i++){
            if (data.data[i].state && data.data[i].state != '正常') {
              status = 1;
            }
