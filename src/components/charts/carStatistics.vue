<template>
  <div class="chart-all">
      ...
      <div class="line-h" id="car-statistics-chart">
      </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    //   seriesData: [
    //     {
    //         data:[200000, 152000, 101000, 260000, 320000]
    //     }
    //   ]
    // };
  ...
  methods: {
    fetchChartData (alias) {
      ...
      return excontrolApi.yibiaosishiGetCLData(param)
    ...
    initChart(axisData, seriesData) {
      const colors = ['#FF9933', '#12B1E1', '#FF6666'];
      ...
      if (!barscase) {
        barscase = Ec.init(document.getElementById('car-statistics-chart'));
      }
      barscase.setOption(option, true);
    },
    setChartResize () {
      let lineDom = document.getElementById('car-statistics-chart');
