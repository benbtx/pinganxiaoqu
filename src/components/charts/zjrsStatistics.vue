<template>
  <div class="chart-all">
      ...
      <div class="line-h" id="zjrs-statistics-chart">
      </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    let resultData = {
      axisData: ['佛教','道教','天主教','基督教','伊斯兰教','其他'],
      seriesData: [
        {
            type:'bar',
            data:[1880, 290, 200, 154, 100, 1233]
        }
      ]
    };
  ...
  methods: {
    ...
    initChart(axisData, seriesData) {
      ...
      if (!barscase) {
        barscase = Ec.init(document.getElementById('zjrs-statistics-chart'));
      }
      barscase.setOption(option, true);
    },
    setChartResize () {
      let lineDom = document.getElementById('zjrs-statistics-chart');
