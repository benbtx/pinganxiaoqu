<template>
  ...
  <div class="mapContainer">
    ...
    <div style="width: 100%; position: absolute; top: 10px;">
        <div class="ssjcPopTool" v-show="!(this.ajaxData.cgq.length==0&&this.ajaxData.sxt.length==0)" >
          <div class="pointSelect tool" title="" >
            <input  ref='all' type="checkbox" name="vehicle" value="all" checked @click="selectAll()">全部
            
          </div>
          <!-- <div class="pointSelect tool" title="" >
            <input  ref='jhsb' type="checkbox" name="vehicle" value="jhsb" checked @click="selectOne('jhsb')">
            <img :src="bmapImg.jhsb" width="16px" height="16px" style="margin-top: -4px;"/>
            净化设备
          </div> -->
          <!-- <div class="boxSelect tool" title="" >
            <input ref='jksxt' type="checkbox" name="vehicle" value="jksxt" checked  @click="selectOne('jksxt')">
            <img :src="bmapImg.jksxt" width="16px" height="16px" style="margin-top: -4px;"/>
            监控摄像头
          </div>

          <div class="boxSelect tool" title=""  >
            <input ref='dlcgq' type="checkbox" name="vehicle" value="dlcgq" checked @click="selectOne('dlcgq')">
            <img :src="bmapImg.dlcgq" width="16px" height="16px" style="margin-top: -4px;"/>电流传感器
          
          </div>
          <div class="boxSelect tool" title=""  >
            <input ref='qtsjcgq' type="checkbox" name="vehicle" value="qtsjcgq" checked  @click="selectOne('qtsjcgq')">
            <img :src="bmapImg.qtsjcgq" width="16px" height="16px" style="margin-top: -4px;"/>
            气体数据传感器
          </div> -->
          ...
          <div class="boxSelect tool" title="" v-show="this.ajaxData.sxt.length!=0">
            <input ref='jksxt' type="checkbox" name="vehicle" value="jksxt" checked  @click="selectOne('jksxt')">
            <img :src="bmapImg.jksxt" width="16px" height="16px" style="margin-top: -4px;"/>
            监控摄像头
          </div>

          <div  v-for="(item) in  this.ajaxData.cgq" :key="item.id" class="boxSelect tool" title=""  >
            <input :ref=item.abbreviation type="checkbox" name="vehicle" :value=item.abbreviation checked  @click="selectOne(item.abbreviation)">
            <img :src="bmapImg[item.abbreviation]" width="16px" height="16px" style="margin-top: -4px;"/>
            {{ item.alias}}
          </div>
    ...
    <!-- <div  class="mapCoordinate" >{{default_config.currentPoint.lng.toFixed(4)+","+default_config.currentPoint.lat.toFixed(4)}}</div> -->
    <div>
...
const cityOptions = ['净化设备', '监控摄像头', '电流传感器', '气体数据传感器'];
export default {
  name: "baidumap",
  data() {
    return {
      ...
      bmapImg: {
        jhsb: "../../../static/image/bmap/jhsb.png",
        dlcgq: "../../../static/image/bmap/dlcgq.png",
        qtsjcgq: "../../../static/image/bmap/qtsjcgq.png",

      
        dlcgq_map: "../../../static/image/bmap/ssjc/dlcgq.png",
        qtsjcgq_map: "../../../static/image/bmap/ssjc/qtsjcgq.png",


       
        dw_map: "../../../static/image/bmap/ssjc/dw.png",
        dw_mapgl: "../../../static/image/bmap/ssjc/dw.png",

       

        jksxt: "../../../static/image/bmap/jksxt.png",
        dqhjwry: "../../../static/image/bmap/dqhjwry.png",
        trzljc: "../../../static/image/bmap/trzljc.png",
        dbsthjwry: "../../../static/image/bmap/dbsthjwry.png",
      ...
      checkAll: true,
        checkedCities: ['净化设备', '监控摄像头', '电流传感器', '气体数据传感器'],
        cities: cityOptions,
  ...
  watch: {
    ...
    //                   if(overlay[j].getLabel().content==that.firstdwInfo.indexCode){
  ...
  created() {
    ...
    // t.filter( item => item.code==31)[0].name
    // this.cgqCodeList.filter( i => i.code==item.transducerType)[0].name||
    //  this.cgqCodeList .filter( item => item.code==31)[0].name
    var that = this;
    ...
    //     //是否高亮点位 id:点位id,value：是否高亮
    // });

    eventBus
      .$off(eventCollection._selectPositionRefresh)
      .$on(eventCollection._selectPositionRefresh, (data) => {
  ...
  methods: {
    handler({ BMap, map }) {
      ...
      if (this.p_mapdefault != undefined) {
        this.default_config.center.lng = this.p_mapdefault.pop_lng;
         this.default_config.center.lat = this.p_mapdefault.pop_lat;
      }
      ...
      //初始摄像头、传感器、点位
      this.getCompanyCamera(this.p_mapdefault.gsid);
      this.getTransducer(this.p_mapdefault.gsid);
      this.getPosition(this.p_mapdefault.gsid);
        ...
        data.forEach((item, idx) => {
          ...
          if(type=="qiyedianwei"){
            marker2 = new this.BMap.Marker(pt, {
              icon: myIcon,
              title:"名称："+item.name+","+item.id//"kakou"
            });
          }else if (type=="cgq"){
            // if(item.transducerType=="65536"){
            //   var i=new this.BMap.Icon(this.bmapImg.dlcgq_map, new this.BMap.Size(24, 24))
            //   i.imageSize = new this.BMap.Size(24, 24);
            //   marker2 = new this.BMap.Marker(pt, {
            //     icon: i,//new this.BMap.Icon(this.bmapImg.dlcgq, new this.BMap.Size(24, 24)),
            //     title:"dlcgq"
            //   });
            // }else if(item.transducerType=="31"){
            //   var i=new this.BMap.Icon(this.bmapImg.qtsjcgq_map, new this.BMap.Size(24, 24))
            //   i.imageSize = new this.BMap.Size(24, 24);
            //   marker2 = new this.BMap.Marker(pt, {
            //     icon:i,//,
            //     title:"qtsjcgq"
            //     // title:"名称："+item.transducerName+","+item.indexCode//"kakou"
            //   });
            // }else{
            //   var i=new this.BMap.Icon(this.bmapImg.qtsjcgq_map, new this.BMap.Size(24, 24))
            //   i.imageSize = new this.BMap.Size(24, 24);
            ...
            // this.cgqStatusList.filter( i => i.flagTag==item.status)[0].name
            // this.cgqCodeList .filter( item => item.code==31)[0].name
            var lx=this.cgqCodeList .filter( i => i.code==item.transducerType)[0].abbreviation

            var i=new this.BMap.Icon(this.bmapImg[lx+"_map"], new this.BMap.Size(24, 24))
            i.imageSize = new this.BMap.Size(24, 24);
            marker2 = new this.BMap.Marker(pt, {
              icon:i,//,
              title:lx
            ...
            let myIcon = new this.BMap.Icon("../../../static/image/bmap/ssjc/dw"+idx+".png", new this.BMap.Size(28, 35));
          ...
          let label = new BMap.Label(
           item.deviceCode || item.indexCode || item.id ||item.indexcode|| item.gatCode ||  item.notificationID||"12",
            {
          ...
          this.setInfowindow(marker2, item, {
            ischeck: false,
            title: "详情",
            type: type!="cgq"?type:(item.transducerType=="65536"?"dlcgq":"qtsjcgq"), //"kakou",
            lx:lx,
            isShowButton: true,
            x: 1
          });
    ...
    refreshMakerts(){
      ...
      //先还原所有数据，不让高亮；然后高亮data中的数据
      var overLay = this.map.getOverlays();
      for (var j = 0; j < overLay.length; j++) {
          if (overLay[j].Bc) {
            if (overLay[j].Bc.className) {
              if( overLay[j].getTitle()=="jksxt"){
                var wgl = new this.BMap.Icon( this.bmapImg.jksxt_map, new this.BMap.Size(24, 24));
                wgl.imageSize = new this.BMap.Size(24, 24);
                overLay[j].setIcon(wgl); //不高亮
                overLay[j].setTop(false);
              }else if(overLay[j].getTitle()=="dw"){
              
                // var url=overLay[j].getIcon().imageUrl;
                // var num=url.split('dw')[1].split('.')[0]
                //  console.log(num);

                // var myIcon = new this.BMap.Icon("../../../static/image/bmap/ssjc/dw"+num+".png", new this.BMap.Size(28, 35));
                // myIcon.imageSize = new this.BMap.Size(28, 35);
                // overLay[j].setIcon(myIcon); //还原点位

              }else{
                var lx=overLay[j].getTitle();//dqhjwry
                overLay[j].getIcon();//"../../../static/image/bmap/ssjc/dlcgq.png"
                var wgl=new this.BMap.Icon(this.bmapImg[lx+"_map"], new this.BMap.Size(24, 24))
                wgl.imageSize = new this.BMap.Size(24, 24);
                overLay[j].setIcon(wgl); //不高亮
                overLay[j].setTop(false);
              }
    ...
    //获取点位设备，并让其高亮
    flashAtMakers: function(data,type) {
      if(type=="sxt"){
          var overlay = this.map.getOverlays();
          for (var j = 0; j < overlay.length; j++) {
            if (overlay[j].Bc) {
              if (overlay[j].Bc.className) {
                if( overlay[j].getTitle()=="jksxt"){
                    for (var k = 0; k < data.length; k++) {
                      overlay[j].getLabel().content
                      if (overlay[j].getLabel().content == data[k].indexCode) {
                          var gl = new this.BMap.Icon( this.bmapImg.jksxt_mapgl, new this.BMap.Size(24, 24));
                          gl.imageSize = new this.BMap.Size(24, 24);
                          overlay[j].setIcon(gl); //不高亮
                          overlay[j].setTop(true);
                      }
                    }
                }
        ...
        for (var j = 0; j < overlay.length; j++) {
          if (overlay[j].Bc) {
            if (overlay[j].Bc.className) {
              if( overlay[j].getTitle()!="jksxt"&&overlay[j].getTitle()!="dw"){
                  for (var k = 0; k < data.length; k++) {
                    overlay[j].getLabel().content
                    if (overlay[j].getLabel().content == data[k].indexCode) {
                        var lx=overlay[j].getTitle();//dqhjwry
                        overlay[j].getIcon();//"../../../static/image/bmap/ssjc/dlcgq.png"
                        var lx=this.cgqCodeList .filter( i => i.code==data[k].transducerType)[0].abbreviation
                        var gl=new this.BMap.Icon(this.bmapImg[lx+"_mapgl"], new this.BMap.Size(24, 24))
                        // var gl = new this.BMap.Icon(data[k].indexCode, new this.BMap.Size(24, 24));
                        gl.imageSize = new this.BMap.Size(24, 24);
                        overlay[j].setIcon(gl); //不高亮
                        overlay[j].setTop(true);
                    }
    ...
    flashAtDWMakers: function(data) {
      var overlay = this.map.getOverlays();
      for (var j = 0; j < overlay.length; j++) {
        if (overlay[j].Bc) {
          if (overlay[j].Bc.className) {
            if(overlay[j].getTitle()=="dw"){
              if (overlay[j].getLabel().content == data.id) {
                if(data.value){
                  ...
                  console.log(num);
                  var myIcon = new this.BMap.Icon("../../../static/image/bmap/ssjc/dw"+num+"_gl.png", new this.BMap.Size(28, 35));
                  myIcon.imageSize = new this.BMap.Size(28, 35);
                  overlay[j].setIcon(myIcon); //还原点位
                  overlay[j].setTop(true);
    ...
    indowindow: function(data, opt) {
      ...
      }else if (opt.type== "jhsb"||opt.type== "jksxt") {
          ...
          <div  class="row">
              <div  class="col-md-12 spandiv">
                <span  class="spandec">设备编码：
                    <span  class="spantext">${data.indexCode||data.indexcode} </span>
                  </span>
              </div>
          ...
          <div  class="row">
              <div  class="col-md-12 spandiv">
                <span  class="spandec">设备状态：
                    <span  class="spantext">${
                      data.isOnline // data.isOnline == 1 ? "在线" : "离线"
                    } </span>
                  </span>
              </div>
      ...
      }else if (opt.type== "dlcgq"||opt.type== "qtsjcgq") {
        sContent += `<div  class="row">
              <div  class="col-md-12 spandiv">
                <span  class="spandec">设备名称：
                    <span  class="spantext">${data.transducerName ||
                      data.name} <span>&nbsp&nbsp<span></span>
                  </span>
              </div>
          </div>
          ...
          <div  class="row">
              <div  class="col-md-12 spandiv">
                <span  class="spandec">设备状态：
                    <span  class="spantext">${
                      data.isOnline||this.cgqStatusList.filter( i => i.flagTag==data.status)[0].description // data.isOnline == 1 ? "在线" : "离线"
                    } </span>
                  </span>
              </div>
    ...
    setInfowindow: function(marker, data, opt) {
      ...
      if(opt.type=="dw"){
        if(this.firstdwInfo==null){
          this.firstdwInfo=data;
           console.log("dw");
            console.log( this.firstdwInfo);
        }
      ...
      marker.addEventListener("click", function(e) {
        ...
        //针对某些负责的infowindow,采用单独的组件进行绘制，且改种形式，地图移动infowindow,不会随之移动 
         if(this._opt.type!=undefined){
            if (this._opt.type== "dw") {
              // alert("触发右边数据刷新");
              this._data;
              eventBus.$emit(eventCollection._positionRefresh, {
                data: this._data,
              });

              that.getPositionDeviceIds(this._data.indexCode)
    ...
    addXzq(){
      var that=this;
      var a=[{xzq:"双流区",color:'red'},
      {xzq:"龙泉驿区",color:'green'},
      // {xzq:"锦江区",color:blue},
      {xzq:"武侯区",color:'blue'}
      ];
      for (var j = 0; j < a.length; j++) {
          ...
          bdary1.get("四川省成都市"+a[j].xzq, function(rs){       //获取行政区域
            ...
            for (var i = 0; i < count; i++) {
              var ply = new BMap.Polygon(rs.boundaries[i], {strokeWeight:3, strokeColor: this.color||"#0b699b",fillColor:this.color||"#3daafc54",fillOpacity:0.2}); //建立多边形覆盖物
    ...
    getCompanyCamera(id) {
      ...
      // let param={};
      // param.parentIndexCode=id;
      return shipinApi
        .getCompanyCameraList(id)
        .then(res => res)
        .then(data => {
          _this.showCompanyCamera(data.data);
           this.ajaxData.sxt=data.data;
        }).catch((err) => {
        // this.$message({
        //   message: '获取公司摄像头失败！',
        //   type: 'error',  
    ...
    showCompanyCamera(data){
      // this.addAtMaker
      this.addAtMaker(data, "jksxt", this.bmapImg.jksxt_map);
    },
      ...
      return shipinApi
        ...
        .then(data => {
          ...
          for (var i in data.data) {
             if(cgqcode.indexOf(data.data[i].transducerType)<0){
               cgqcode.push(data.data[i].transducerType);
             }
          }

          for (var i in cgqcode) {

            for (var j in _this.cgqCodeList) {
              if(cgqcode[i]==_this.cgqCodeList[j].code){
                _this.ajaxData.cgq.push(_this.cgqCodeList[j]);
              }
            }
          ...
          this.addAtMaker(data.data, "cgq", this.bmapImg.jksxt);
    ...
    getPosition(id) {
      ...
      return shipinApi
        ...
        .then(data => {
          // console.log(data);
            this.addAtMaker(data.data, "dw", this.bmapImg.dw_map);
        }).catch((err) => {
        this.$message({
          message: '获取公司点位失败！',
          type: 'error',  
        ...
        .then(data => {
          ...
          if (data.camera) {  
            if(data.camera.length!=0){
              this.flashAtMakers(data.camera,"sxt");  
            } 
          }
          if (data.transducer) { 
            if(data.transducer.length!=0){
               this.flashAtMakers(data.transducer,"cgq");
            }   
          }
        }).catch((err) => {
          // this.$message({
    ...
    selectAll(){
      ...
      // if(this.$refs.all.checked){
      //   //全选中
      //   // this.$refs.jhsb.checked=true;
      //   this.$refs.jksxt.checked=true;
      //   this.$refs.dlcgq.checked=true;
      //   this.$refs.qtsjcgq.checked=true;
      //   //全部显示
      //   // if(this.haveMaker("jhsb")){
      //   //     this.showAtMaker("jhsb");
      //   // }
      //   if(this.haveMaker("jksxt")){
      //       this.showAtMaker("jksxt");
      //   }
      //   if(this.haveMaker("dlcgq")){
      //       this.showAtMaker("dlcgq");
      //   }
      //   if(this.haveMaker("qtsjcgq")){
      //       this.showAtMaker("qtsjcgq");
      //   }


      // }else{
      //   //全不选中
      //   // this.$refs.jhsb.checked=false;
      //   this.$refs.jksxt.checked=false;
      //   this.$refs.dlcgq.checked=false;
      //   this.$refs.qtsjcgq.checked=false;
      //   // if(this.haveMaker("jhsb")){
      //   //     this.hideAtMaker("jhsb");
      //   // }
      //   if(this.haveMaker("jksxt")){
      //       this.hideAtMaker("jksxt");
      //   }
      //   if(this.haveMaker("dlcgq")){
      //       this.hideAtMaker("dlcgq");
      //   }
      //   if(this.haveMaker("qtsjcgq")){
      //       this.hideAtMaker("qtsjcgq");
      //   }
      // }
    ...
    selectOne(lx){
      if(lx=="jksxt"){
        if(this.$refs.jksxt.checked==true){
           this.showAtMaker(lx);
        }else{
           this.hideAtMaker(lx);
        }
           ...
           //判断当前是否选中，没
          if(this.$refs[lx][0].checked){
            if(this.haveMaker(lx)){
              this.showAtMaker(lx);
            }
          }else{
            if(this.haveMaker(lx)){
      ...
      Object.getOwnPropertyNames(this.$refs).forEach(function(key){
         ...
         if(key!="all"){
           if(key=="jksxt"){
              if(this.$refs[key].checked==false){
                this.$refs.all.checked=false;
              }else{
                sjcount+=1;
              }
           }else{
              if(this.$refs[key][0].checked==false){
                this.$refs.all.checked=false;
              }else{
                sjcount+=1;
              }
      ...
      //如果四个都选中，则全部那个也选中；有一个没选中，全部就取消选中

      // if(this.$refs.jksxt.checked==true && this.$refs.dlcgq.checked==true&&this.$refs.qtsjcgq.checked==true ){
      //   this.$refs.all.checked=true;
      // }
      // if(this.$refs.jksxt.checked==false|| this.$refs.dlcgq.checked==false|| this.$refs.qtsjcgq.checked==false ){
...
.dlcgq{
  background-image: url(../../../static/image/bmap/dlcgq.png)
}
.ssjcPopTool {
  /* position: absolute;
  top: 20px; */
