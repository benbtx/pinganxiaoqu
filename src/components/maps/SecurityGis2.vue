...
export default {
  ...
  methods: {
    ...
    ShowPerceptualSource: function(dtObj) {
      ...
      if (ischeck == false) {
        if (layerid == 'camera')
          mapVar.removeMapOverlays(this.gzy_showList['tw']);
        else if (layerid == 'police')
          mapVar.removeMapOverlays(this.gzy_showList['jl']);
        else if (layerid == 'wifi')
          mapVar.removeMapOverlays(this.gzy_showList['wifi']);
        else if (layerid == 'specialWifi')
          mapVar.removeMapOverlays(this.gzy_showList['specialWifi']);
        return;
      }
      let _this = this;
      /**显示安保圈感知源*/
      var show_abqGzy = function(d) {
        ...
        var add = function(data, tp, mUrl, cb) {
          ...
          var p = function(atm) {
            setTimeout(() => {
              ...
              _this.gzy_showList[tp].push(marker);
        ...
        var spaceQuery = function(layername, p = poly.join(',')) {
          ...
          var callback = function(data) {
            ...
            pgisMap.openInfoWindow(appMap.map, popData, '');
          };
    ...
    showCircles: function(e) {
      ...
      if (typeof e == 'number') {
        ...
        if (item) {
          ...
          // mapVar.mapInstance[0].setMapCenter(item.lon, item.lat, 19);
          appMap.setMapCenter(item.lon, item.lat, 19);
      ...
      if (this.curSecurity_name.indexOf(id) < 0) {
        ...
        if (item.lon && item.lat) appMap.setMapCenter(item.lon, item.lat, 19);
        return;
      }
...
//将安保规模的的 文字转换成数字编码
      ...
      for (var i = 0; i < datas.length; i++) {
        ...
        if (
         ...
         oneData.lon == null ||
          ...
          this.createOverHtml(
                oneData.activityid,
                colorObj[oneData.status],
                titleObj[oneData.peoplenum],
                oneData.name,
                mimg[oneData.status],
                pos2,
                oneData.peoplenum,
          ...
          // if (oneData.status == 0) {
          //   //5W+
          //   if (oneData.peoplenum == 1) {
          //     this.createOverHtml(
          //       oneData.activityid,
          //       colorObj.over,
          //       titleObj[oneData.peoplenum],
          //       oneData.name,
          //       mapVar.imgFactory.over,
          //       pos2,
          //       oneData.peoplenum,
          //       this.showCircles
          //     );
          //   } else if (oneData.peoplenum == 2) {
          //     //3-5W+
          //     this.createOverHtml(
          //      oneData.activityid,
          //       colorObj.over,
        ...
        if (histDataPtInfo) {
          ...
          if (histDataPtInfo.type == 'jcss') {
            ...
            if (!!marker) {
              ...
              //     let t = type == 'jcss' ? "天网" : "WIFI";
              //     let html = '<div class="ctn-hold">' +
              //         '<div class="place-hold">' +
              //         '<p class="para-normal key">设备编号:<span class="value"> ' + histDataPtInfo.id + '</span></p>' +
              //         '<p class="para-normal key">所在位置：<span class="value"> ' + histDataPtInfo.title + '</span></p>' +
              //         '<p class="para-normal key">设备类型:<span class="value"> ' + t + '</span></p>' +
              //         '</div></div>';
              //     marker.openInfoWindowHtml(html);
            ...
            if (!!marker) {
              marker.addListener('click', e => {
                appMap.setMapCenter(
                  e.latlng.lng,
                  e.latlng.lat,
                  appMap.map.getZoomLevel() + 1
                );
              });
              // marker.addListener('mouseover',function(e){
              //     let t=histDataPtInfo.type=='jcss'? "天网":"WIFI";
              //     let html = '<div class="ctn-hold">' +
              //         '<div class="place-hold">' +
              //         '<p class="para-normal key">设备编号:<span class="value"> ' + histDataPtInfo.id + '</span></p>' +
              //         '<p class="para-normal key">所在位置：<span class="value"> ' + histDataPtInfo.title + '</span></p>' +
          ...
          if (lineData.length >= 2) {
            ...
            appMap.map.addOverlay(histLineTrack);
          }
        }
      }
    ...
    changeMarker: function(data) {
      if (mapVar.security_ABpoint && mapVar.security_ABpoint.length) {
        ...
        if (m) appMap.map.removeOverlay(m.m);
      }
    ...
    hl_twinkle: function(e) {
      ...
      let item = mapVar.security_twinkle.find((item, index) => {
        ...
        //    // appMap.map.removeOverlay(item.d);
        //    console.log(yjid);
    ...
    isInWindowView: function(lng, lat) {
      let max_lat = appMap.map.getBoundsLatLng()._northEast.lat;
      let min_lat = appMap.map.getBoundsLatLng()._southWest.lat;
      let max_lng = appMap.map.getBoundsLatLng()._northEast.lng;
      let min_lng = appMap.map.getBoundsLatLng()._southWest.lng;
      if (
        lat <= max_lat &&
        lat >= min_lat &&
        lng >= min_lng &&
    ...
    setSensitive_Circle: function(circleData) {
      ...
      //console.log(ci);
      appMap.map.addOverlay(co);
      mapVar.overLayer_security.push(co);
      appMap.map.addOverlay(ci);
    ...
    clearDraw: function() {
      ...
      mapVar.removeMapOverlays(mapVar.overLayer_abqgzy);
